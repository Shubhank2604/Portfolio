...
const links = [{ label: "Experience", href: "/#experience" }, { label: "Projects", href: "/projects/" }, { label: "Writing", href: "/writing/" }, { label: "About", href: "/#about" }];
export function Navbar() {
  ...
  </div><nav id="mobile-navigation" className={`mobile-nav ${open ? "open" : ""}`} aria-label="Mobile navigation">{links.map((link) => <Link key={link.label} href={link.href} onClick={() => setOpen(false)}>{link.label}</Link>)}<Link href="/resume.pdf" target="_blank" onClick={() => setOpen(false)}>Resume ↗</Link><ThemeToggle /></nav></header>;
}
